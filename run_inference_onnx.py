...
# -*- coding: utf-8 -*-
"""
数字人 ONNX 推理一键脚本：提取音频特征 -> ONNX 视频推理 -> 音视频合成
"""
import argparse
import os
import subprocess
...
def get_args():
    parser = argparse.ArgumentParser(description='数字人 ONNX 推理：输入音频生成口型视频')
    ...
    parser.add_argument('--onnx', type=str, required=True, help='unet.onnx 模型路径')
...
def main():
    ...
    ret = subprocess.run(
        [
            sys.executable, os.path.join(project_root, 'inference_onnx.py'),
            '--asr', args.asr,
